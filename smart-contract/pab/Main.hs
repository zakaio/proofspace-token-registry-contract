...
instance Builtin.HasDefinitions StarterContracts where
    getDefinitions = [TokenRegistryContract]
    getSchema =  \case
        TokenRegistryContract -> Builtin.endpointsToSchemas @TokenRegistry.TokenRegistrySchema
    getContract = \case
        TokenRegistryContract -> SomeBuiltin (TokenRegistry.tokenRegistryAddress @ContractError)
